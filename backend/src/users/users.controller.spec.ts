...
describe('usersController', () => {
  ...
  it('users create failed 0', async () => {
    const res = mockResp();
  ...
  it('users create failed 1', async () => {
    const res = mockResp();
  ...
  it('users create failed 2', async () => {
    const res = mockResp();
  ...
  it('users create failed 3', async () => {
    const res = mockResp();
  ...
  it('users create failed 4', async () => {
    const res = mockResp();
  ...
  it('reservation cancel failed 1', async () => {
    ...
    req.user.id = 1403;
    req.user.role = 1;
    await usersController.cancel(req, res, next);
    expect(res.status.mock.calls[0][0]).toBe(status.BAD_REQUEST);
    expect(res.json.mock.calls[0][0]?.errorCode).toBe(1);
  });
  ...
  it('reservation count (ok)', async () => {
    ...
    req.user.role = 0;
    await usersController.count(req, res, next);
    expect(res.status.mock.calls[0][0]).toBe(status.OK);
  ...
  it('get user reservation (OK)', async () => {
    const res = mockResp();
    req.user.id = 1412;
    await usersController.userusers(req, res, next);
    expect(res.status.mock.calls[0][0]).toBe(status.OK);
